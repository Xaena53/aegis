...
curl http://localhost:8788/health          # -> {"ok":true,"sessions":0}
```

Then open <http://localhost:8788/connect> to link a Google Ads account.

## Plain Docker

```bash
docker build -t adspilot .
docker run -d --name adspilot -p 8788:8788 \
  --env-file .env -e PORT=8788 -e ADSPILOT_DB=/data/adspilot.db \
...
| `ADSPILOT_PUBLIC_URL` | recommended | Externally visible URL; Host/Origin validation and OAuth redirects derive from it. Compose defaults it to `http://localhost:8788`. |
| `PORT` | no | Image default **8788**. Compose pins it — change the *left* side of the port mapping instead. |
...
| Every `/mcp` request returns 403 | `ADSPILOT_PUBLIC_URL` (or `ADSPILOT_ALLOWED_HOSTS`) doesn't match the Host the client uses — e.g. URL says `:8787` but you mapped `:8788`. Behind nginx, keep `proxy_set_header Host $host;`. |
